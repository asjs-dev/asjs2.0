...
roFunc( ASJS.GeomUtils, "pointInOrigo", function( point ) {
	return point.x == 0 && point.y == 0;
});

roFunc( ASJS.GeomUtils, "twoPointDistance", function( pointA, pointB ) {
	return Math.sqrt( Math.pow( pointB.x - pointA.x, 2 ) + Math.pow( pointB.y - pointA.y, 2 ) );
});

roFunc( ASJS.GeomUtils, "pointInRect", function( point, rect ) {
	return point.x >= rect.x && point.x <= rect.x + rect.width && point.y >= rect.y && point.y <= rect.y + rect.height;
});

roFunc( ASJS.GeomUtils, "twoPointEquals", function( pointA, pointB ) {
	return pointA.x == pointB.x && pointA.y == pointB.y;
});

roFunc( ASJS.GeomUtils, "twoPointAngle", function( pointA, pointB ) {
...
roFunc( ASJS.GeomUtils, "globalToLocal", function( target, point ) {
	var pos = new ASJS.Point( point.x, point.y );
	var child = target;
	var children = [ child ];
	while ( child ) {
...
roFunc( ASJS.GeomUtils, "hitTest", function( target, point ) {
